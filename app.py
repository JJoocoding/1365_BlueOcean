...
def parse_api_header_from_json(data):
    """공공데이터포털 JSON 응답에서 header.resultCode/resultMsg 추출"""
    try:
...
def parse_api_header_from_xml(data):
    """공공데이터포털 XML 응답에서 header.resultCode/resultMsg 추출"""
    try:
        response = data.get("response", {})
        header = response.get("header", {})
        code = header.get("resultCode")
...
def fetch_json(url: str, desc: str, api_warnings: list, timeout: int = 10):
    """
    JSON API 공통 호출 함수
    - HTTP 오류, JSON 파싱 오류, resultCode≠00 모두 api_warnings에 추가
    - 정상일 때 data 리턴, 오류 시 None 리턴
    """
    try:
    ...
    if code is not None and code != "00":
        api_warnings.append(
            f"[API 오류] {desc} (resultCode={code}, resultMsg={msg})"
        )
        return None
...
def analyze_winrate_distribution(winner_rates, hot_start, hot_end):
    ...
    results = []
    total = len(winner_rates)  # 시도 = 분석 공고수
...
def render_winrate_panel(winner_rates, hot_start, hot_end):
    ...
    st.markdown(
        ...
        <div style="padding:12px; border-radius:10px; 
            ...
            <p style="color:#ccc; margin-top:4px;">과거 1순위 사정율이 어느 구간에서 주로 터졌는지 보여주는 보조지표입니다.</p>
