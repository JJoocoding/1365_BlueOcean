...
def find_blue_ocean_v2(theoretical_rates, bidder_rates, hot_start, hot_end, bin_width=0.05):
    """
    🔵 블루오션 정의 (최종 버전)
    1) 사정율이 많이 몰리는 구간(핫존) 안에 있어야 한다.
    2) 그 구간에 '이론상 1365 조합은 많이 몰렸는데' 실제 투찰 업체 수는 상대적으로 적다.

    S(i) = (정규화된 1365 밀도) × (정규화된 1 / 투찰업체 밀도)
    """
    ...
    bid_norm = bid_counts / bid_counts.sum() if bid_counts.sum() > 0 else np.zeros_like(bid_counts)
    ...
    for i in range(len(bin_edges) - 1):
        ...
        # S(i) = (P_theo / maxP_theo) * ((minP_bid+eps) / (P_bid+eps))
        demand = p_theo / max_theo
        supply_inverse = (min_bid + eps) / (p_bid + eps)
        score = demand * supply_inverse

        if score <= 0:
            continue

        rows.append({"center": center, "score": score})
...
def analyze_gongo(gongo_input_str: str):
    ...
    try:
        ...
        if not df4.empty and "bidprcAmt" in df4.columns:
            ...
            if not df4.empty:
                # 1순위 업체 (API 정렬 상 1행이 1순위인 경우가 많음)
...
def process_analysis(target_officer: str, gongo_input: str):
    ...
    # 블루오션 (theo + bidder 기반)
    blue_df, best_range, best_center = find_blue_ocean_v2(
        theoretical_rates_all, bidder_rates_all, hot_start, hot_end, bin_width=0.05
    )
    ...
    if blue_df is not None and best_range is not None:
        ...
        rec_rate = round(best_center, 4) if best_center is not None else None
        ...
        blue_desc = (
            ...
            f"  1365 이론 조합은 많이 몰렸지만 실제 투찰 업체 수는 상대적으로 적은\n"
...
with btn_col2:
    reset_clicked = st.button("🧹 초기화", use_container_width=True, on_click=reset_gongo)
